...
const renderCalendar = () => {
    ...
    const datePreset = [];
    for(let i = 0; i < vacinationCollection?.length; i++ ) {
        datePreset.push(`${vacinationCollection[i].getDate()}-${vacinationCollection[i].getMonth() + 1}-${vacinationCollection[i].getFullYear()}`)
    }
    ...
    const getCheckedClass = (prevDateKey) => {
        ...
        return datePreset.includes(prevDateKey) ? 'is-checked-for-vacination' : '';
    }
...
document.getElementById('old-calculate-btn').addEventListener('click', function () {
    ...
    if (oldBirthdateValue) {
        ...
        const calculatedFirstWeekVaccine    = calculateWeeksAfterDate(oldBirthdate, 6).toLocaleDateString('bg-BG');
        const calculatedSecondWeekVaccine   = calculateWeeksAfterDate(oldBirthdate, 8).toLocaleDateString('bg-BG');
        const calculatedThirdWeekVaccine    = calculateWeeksAfterDate(oldBirthdate, 9).toLocaleDateString('bg-BG');
        const calculatedFourthWeekVaccine   = calculateWeeksAfterDate(oldBirthdate, 12).toLocaleDateString('bg-BG');
        const calculatedWeekRevaccine       = calculateWeeksAfterDate(oldBirthdate, 52).toLocaleDateString('bg-BG');

        document.getElementById('oldvaccine1').textContent  = calculatedFirstWeekVaccine;
        document.getElementById('oldvaccine2').textContent  = calculatedSecondWeekVaccine;
        document.getElementById('oldvaccine3').textContent  = calculatedThirdWeekVaccine;
        document.getElementById('oldvaccine4').textContent  = calculatedFourthWeekVaccine;
        document.getElementById('oldrevaccine').textContent = calculatedWeekRevaccine;

    
        vacinationCollection = [
            calculateWeeksAfterDate(oldBirthdate, 6),
            calculateWeeksAfterDate(oldBirthdate, 8),
            calculateWeeksAfterDate(oldBirthdate, 9),
            calculateWeeksAfterDate(oldBirthdate, 12),
            calculateWeeksAfterDate(oldBirthdate, 52)
        ];
